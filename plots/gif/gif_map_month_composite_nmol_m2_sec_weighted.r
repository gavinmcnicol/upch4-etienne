#  Currently this composite is made by multiplying the unweighted monthly composite
# with a monthly composite of Fw.  
# Instead, we should do the multiplication at monthly timeestep
...
for (t in seq(1,12)){ #length(names(flux))){
	...
	m <- ggplot() +
		...
		# scale_fill_gradientn(colors= rev(sequential_hcl(10, palette = 'ag_Sunset')), #low="#fffcba", high="#e80000", 
		# 					trans="log", 
		# 					breaks=c(10^0, 10^1, 10^2),
		# 					#labels=c(10^-5, 10^-4, 10^-3, 10^-2),
		# 					labels=c(expression(10^{0}),
		# 					         expression(10^{1}),
		# 					         expression(10^{2})),
...
},  movie.name = "/home/groups/robertj2/upch4/output/figures/monthly_ch4_compositech4_v200_nmol_m2_sec_weighted_v2.gif", 
