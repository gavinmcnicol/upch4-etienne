...
upch4_wad2m_sum_df_deg <- ts2monthdegsumfluxsinelat(upch4_sum)
...
# Convert raster stack to degree sums df
upch4_giems2_sum_df_deg <- ts2monthdegsumfluxsinelat(upch4_sum)
...
wc_df_deg <- ts2monthdegsumfluxsinelat(wc_11)
...
# JUNE 2022 - FOR SOME FUCKING REASON THE SCRIPT CALCULATING THIS HAS DISAPPEARED AND 
#             THERE IS NO SAVED FILE OF TG PER MONTH GRIDS...
...
r <- gcp_ts_05 #gcp_ts_1

# multiply by m^2 area
r <- r * area(r[[1]]) * 1e+6

# convert  day to month
r <- r * 30.4167  # 0.0328767

# Convert mg to Tg
r <- r * 1.0E-15

# sum(cellStats(r[[1:12]], sum))
gcp_ts_1 <- r

gcp_ts_1_df_deg <- ts2monthdegsumfluxsinelat(gcp_ts_1)
...
# Convert raster stack to degree sums
td_df_deg <- ts2monthdegsumfluxsinelat(td_11)
# x<-td_11
...
sum(td_df_deg$sum_Tgmonth, na.rm=T)
...
# Maximum Tg/month/1deg value?
maxcap= 1.4 # 1.0
# Make facet plot
monthly_heatmap_facet_flux <- make_flux_heatmap_facet_sinelat(sum_df_deg, 'sum_Tgmonth', 'model')
...
maxcap= 1.0
# Make facet plot
monthly_heatmap_facet_scaled <- make_flux_heatmap_facet_sinelat(sum_df_deg, 'sum_Tgmonth_scaled', 'model')
...
# maxcap= 1.0
# Make facet plot
monthly_heatmap_facet_perc <- make_perc_heatmap_facet_sinelat(sum_df_deg, 'sum_Tgmonth_perc', 'model')
...
monthly_heatmap_facet_diff <- make_diff_heatmap_facet_sinelat(sum_df_deg_diff, 'diff_Tgmonth', 'model')
...
ggsave('../output/figures/heatmap/heatmap_multifacetpanel_v10c.png',
       d, width=195, height=200, dpi=300, units='mm')


ggsave('../output/figures/heatmap/heatmap_multifacetpanel_v10c.pdf',
