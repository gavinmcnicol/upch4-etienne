...
bioclim_list <- list.files(path = "./Data/Bioclimatic", 
                           full.names = TRUE, 
                           pattern = ".tif")
...
bioclim_stack <- raster::aggregate(bioclim_stack, fact = 6, fun = mean)
...
coords_acquired <- cbind(towers_acquired$LON, towers_acquired$LAT)
...
towers_coords_df_acquired <- cbind(towers_coords_df_acquired, 
                                   raster::extract(bioclim_stack, towers_coords_acquired))
...
# Get coordinates from the tower data
coords_all <- cbind(towers_all$LON, towers_all$LAT)
...
# Convert bioclimatic stack data from wide format to LON format
# Purpose of this is to plot cumulative frequency
bioclim_stack_df2 <- bioclim_stack_df %>%
                     tidyr::gather(key = bioclim_var, 
                                   value = value, 
                                   wc2.0_bio_5m_01:tmp_avgr)

# Convert towers stack data from wide format to LON format
