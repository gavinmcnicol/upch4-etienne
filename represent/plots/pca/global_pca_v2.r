...
names(pcainput) <- c('EVI','SRWI','LE','MAT')
...
towers_u <- bind_cols(towers_vars_filt[,c('SITE_ID','SITE_CLASSIFICATION','Ann_Flux_g_CH4.C_m.2')], towers_u )
...
pca_plot = ggplot() +
	...
	geom_segment(data=vscores, aes(x=0, xend=PC1*0.6, y=0, yend=PC2*0.7), arrow = arrow(length = unit(0.3, "cm")), colour="black", size=0.15) + 
	...
	geom_point(data = towers_u, aes(x = PC1, y = PC2, fill=Ann_Flux_g_CH4.C_m.2, shape=SITE_CLASSIFICATION), 
			   color='black', size = 2.1, stroke=0.25) +  # shape=21,   
	scale_fill_distiller(palette = "Blues", direction=1, limits = c(0, 115), breaks=c(0,25,50,75,100,115)) + 
	guides(fill = guide_colorbar(title = expression(paste("gC m"^2*" yr"^-1))),
		   shape = guide_legend(title  = 'Wetland\nclass')) +

	# add labels
	geom_text_repel(data= subset(towers_u, (PC1>0.1 | PC1< -0.1 | PC2 > 0.1 | PC2< -0.1) ),
									aes(label=SITE_ID, x=PC1, y=PC2), 
									size = 1.8, 
									colour='blue', 
									segment.size = 0.15, 
									segment.color='blue',
	...
	xlab('PC1 (59.2%)') + ylab('PC2 (33.6%)') +
	line_plot_theme +
	theme(panel.border = element_rect(color = "black", fill = NA, size = 0.3),
				legend.position = 'top') #c(0.025, 0.18))


# Save settings for the MDS plot 
ggsave(	plot = pca_plot, 
		file = "pca_all_preds_4preds_v4_NOV2020_v5.pdf", 
		path = "../output/figures/representativeness", 
		width = 87, height = 105, dpi = 400, units = "mm")
