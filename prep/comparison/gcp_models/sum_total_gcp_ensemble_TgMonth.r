...
# Misc function summing raster values
...
date_ls <- seq(as.Date("2001/1/15"), by = "month", length.out = 156) #216)
...
for (i in gcp_list){
	...
	# print time ticker
	# print(paste0(as.Date("2000-01-01") %m+% months(t-1), "  ---  step: ", i))
	print(i)

	# read ch4 flux from NCDF as raster
	# r <- raster(paste0(p, i), varname = "CH4_e", band = t)
	r <- brick(paste0(p, i), varname = "CH4_e", band = seq(12, 204))
	...
	temp_df = as.data.frame(cellStats(r, sum))
	names(temp_df) <- c('flux')
	temp_df$model <- i
	temp_df$date <- date_ls
	temp_df$units <- nc_open(paste0(p, i))$var$CH4_e$units
	...
	gcp_ch4_model_sum <- bind_rows(gcp_ch4_model_sum, temp_df) 
	                               # list(model=i, 
	                               #      date=date_ls,  #as.Date("2000-01-01") %m+% months(t-1),
	                               #      flux=sum_raster(r), 
	                               #      units=units))
	}
...
write.csv(gcp_ch4_model_sum, "../output/results/gcp_ch4_model_sum_2000_2017_indiv.csv")
