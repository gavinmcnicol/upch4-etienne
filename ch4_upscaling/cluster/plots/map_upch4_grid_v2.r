...
setwd("/Users/etiennefluet/Dropbox/GCP_Stanford_Projects/scripts/ch4_upscaling/cluster/plots")

# Get mapping theme
source("../theme_gif_map.r")
...
flux <- brick('./upch4_med_nmolm2sec.nc')# , varname="upch4")
# replace negative (sink) values
flux[flux < 0] = NA
flux[flux < lw_cut] = NA

# Get date list
parseddates <- readRDS('./parsed_dates.rds')
...
# for (t in 1:(length(names(flux)))){
	...
	m <- ggplot() +
	  ...
	  geom_polygon(data=coastsCoarse_df, aes(long, lat, group=group), 
		...
		# Coastline
		geom_path(data=coastsCoarse_df, aes(long, lat, group=group), 
		          color='black', size=0.07) +
		
		# Tower sites
		# geom_point(data=bams_towers, aes(Longitude, Latitude),color='black', fill= "green", shape=21,  size=0.4, stroke=0.1) +
		...
		scale_fill_gradient(low="#fffcba", high="#ad0000", 
							 trans="log", 
							 #breaks=c(lw_cut, 0.001, 0.005, 0.01, 0.02),
							 breaks=c(10^-6, 10^-5, 10^-4, 10^-3, 10^-2),
							 labels=c(10^-6, 10^-5, 10^-4, 10^-3, 10^-2),
							 #labels=c(lw_cut, 0.001, 0.005, 0.01, 0.02),
		...
		guides(fill = guide_colorbar(nbin=15, raster=F, 
		                             barheight = 10,barwidth=1, 
		                             frame.colour=c("black"), frame.linewidth=1, 
