...
diff_tg_per_m2 <- diff / a_m2 

diff_g_per_m2 <- diff_tg_per_m2 * 1e+12
...
#diff_df$layer_cut <- cut(diff_df$layer, c(-120, -75, -50, -25, -1, 1, 25, 50, 75, 120))
#diff_df$layer_cut <- cut(diff_df$layer, c(-120, -60, -30, -15, -1, 1, 15, 30, 60, 120))
diff_df$layer_cut <- cut(diff_df$layer, c(-120, -50, -25, -10, -1, 1, 10, 25, 50, 120))
...
sites_updated <- read.csv("../data/tower_sites/Sites_all_BAMS_15march2019_v2.csv")
...
source('./plots/get_coast_poly_worldmap.r')
...
ggplot() +
  ...
  scale_fill_brewer(palette = "RdBu", drop = FALSE, name=expression("gCH4 m-2 yr-1"^-1)) +
...
ggsave('../output/figures/fig1_map/map_td_bu_difference_gperm2_v10.pdf', 
